...
class UserController extends Controller
{
    public function index(){
        if(Auth::check()){
            ...
            $user_list = User::where('type', '2')->select('users.id','users.status','users.name','users.email','users.phone_number','company.name As branchname')->join('user_detail', 'users.id', '=', 'user_detail.user_id')->join('company', 'users.branch_id', '=', 'company.id')->paginate(10);

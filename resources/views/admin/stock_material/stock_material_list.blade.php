                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach ($stock_material_list as $stock_material)
                                            <tr>
                                                ...
                                                <td>{{ $stock_material->machine_name }}</td>
                                                @php
                                                    $stock_report_color = DB::table('stock_meterial_color')
                                                                    ->select('stock_meterial_color.color_id','stock_meterial_color.stock_material_id','stock_meterial_color.color_item_id','stock_meterial_color.gram','color.color_name')
                                                                    ->where('stock_meterial_color.stock_material_id',$stock_material->id)
                                                                    ->join('color', 'stock_meterial_color.color_item_id', '=', 'color.id')
                                                                    ->get();
                                                @endphp
                                                <td>
                                                    @php $colorName = []; @endphp
                                                    @foreach($stock_report_color as $key => $value)
                                                            @php 
                                                                $colorName[] = $value->color_name;
                                                            @endphp
                                                    @endforeach
                                                    @php
                                                        $colorNamestring = implode(' ', $colorName);
                                                    @endphp
                                                    {{ $colorNamestring }}
                                                </td>
                                                <td>
                                ...
                                <div class="d-flex justify-content-center">
                                    {{--  {!! $stock_material_list->links('pagination::bootstrap-4') !!}  --}}
                                </div>
                            </div>
                            <!-- /.card-body -->
                        </div>
